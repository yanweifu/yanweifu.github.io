# Prune Model
This is the main code for pruning model, for detailed code please refer to download.
...
net_dict = {'conv1.out' : 1*2, 'conv2.in' : 1*2 , 'conv2.out' : 1*2,'fc.in':49 *2}
model = grow_net.Two_Conv_Net(**net_dict).to(device)
if args.parallel:
...
optimizer.prune_layer_by_order_by_name(90, 'fc.weight')
```